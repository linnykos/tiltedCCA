context("Test .search_distinct_perc")

compute_search_distinct_perc_ingredients <- function(setting = 1){
  # setting 1 is where modality 2 has no information
  if(setting == 1){
    n_clust <- 100
    high <- 0.9; low <- 0.05
  ...
  frnn_union <- .compute_frnn_union(svd_1, svd_2,
                                    num_neigh = num_neigh,
                                    radius_quantile = 0.5)
...
test_that(".search_distinct_perc works", {
  set.seed(10)
  tmp <- compute_search_distinct_perc_ingredients(setting = 2)
  score_1 <- tmp$score_1
  score_2 <- tmp$score_2
  frnn_union <- tmp$frnn_union
  basis_list <- tmp$basis_list
  num_neigh <- tmp$num_neigh
  svd_1 <- tmp$svd_1
  svd_2 <- tmp$svd_2
  
  res <- .search_distinct_perc(num_neigh = num_neigh,
                               score_1 = score_1,
                               score_2 = score_2,
                               svd_1 = svd_1,
                               svd_2 = svd_2,
