...
## postprocess_variable_selection is correct

test_that("postprocess_variable_selection works", {
  ...
  res <- postprocess_variable_selection(input_obj = multiSVD_obj,
                                        input_assay = 2,
  ...
  expect_true(all(sort(names(res)) == sort(c("selected_variables", "candidate_list"))))
