context("Test min embedding")

compute_min_embedding_ingredients <- function(setting = 1){
...
## .compute_metacluster_subsample is correct

test_that(".compute_metacluster_subsample works", {
  set.seed(10)
  metacell_clustering_1 <- factor(sample(c("a","b","c"), size = 100, replace = T))
  metacell_clustering_2 <- factor(sample(c("i","ii","iii"), size = 100, replace = T, prob = c(1,2,3)))
  nn_vec_1 <- sample(1:100, 10)
  nn_vec_2 <- sample(1:100, 10)
  num_neigh <- 10
  
  res <- .compute_metacluster_subsample(metacell_clustering_1 = metacell_clustering_1,
                                        metacell_clustering_2 = metacell_clustering_2,
                                        nn_vec_1 = nn_vec_1,
                                        nn_vec_2 = nn_vec_2,
                                        num_neigh = num_neigh)
  expect_true(is.numeric(res))
  expect_true(length(res) == num_neigh)
})

###################

## compute_min_subspace is correct

test_that("compute_min_subspace works", {
  set.seed(10)
  tmp <- compute_min_embedding_ingredients(1)
  ...
  min_subspace <- compute_min_subspace(dimred_1 = .mult_mat_vec(svd_1$u, svd_1$d),
                                       ...
                                       num_neigh = 30,
                                       verbose = F)
  expect_true(is.list(min_subspace))
  expect_true(all(sort(names(min_subspace)) == sort(c("min_mat", "subspace_mat"))))
  
  ## min_mat <- as.matrix(min_subspace$min_mat); image(min_mat, asp = T)
  ## zz <- min_subspace$subspace_mat; plot(zz[,1], zz[,2], asp = T, col = rep(1:3, each = 100))
})
