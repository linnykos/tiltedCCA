.common_decomposition <- function(score_1, score_2, nn_1, nn_2){
  rank_c <- min(ncol(score_1, score_2))
  sapply(1:rank_c, function(k){
    ...
    if(sum(abs(vec_right - vec1)) <= sum(abs(vec_right - vec1))){
      common_vec <- .angle_from_vector(vec_right, (1-common_perc)*ang)
    } else {
      common_vec <- .angle_from_vector(vec_right, common_perc*ang)
    }
    
    a <- .l2norm(common_vec)^2; b <- -.as.numeric(common_vec %*% (vec1 + vec2)); c <- as.numeric(vec1 %*% vec2)
    .quadratic(a,b,c) * common_vec
  })
}
...
.latent_common_perc <- function(score_vec_1, score_vec_2, nn_1, nn_2){
  ...
  mode1_common_perc <- sapply(1:n, function(i){
    val_1in1 <- stats::sd(score_vec_1[i] - score_vec_1[nn_1$nn.idx[i,]])
    val_1in2 <- stats::sd(score_vec_1[i] - score_vec_1[nn_2$nn.idx[i,]])
    val_2in1 <- stats::sd(score_vec_2[i] - score_vec_2[nn_1$nn.idx[i,]])
    val_2in2 <- stats::sd(score_vec_2[i] - score_vec_2[nn_2$nn.idx[i,]])
