...
dmca_decomposition <- function(dmca_res, verbose = T){
  ...
  common_mat_1 <- dmca_res$common_score_1 %*% coef_1
  common_mat_2 <- dmca_res$common_score_2 %*% coef_2
  
  if(verbose) print("D-MCA: Computing distinctive matrices")
  distinct_score_1 <- dmca_res$score_1 - dmca_res$common_score_1
  distinct_score_2 <- dmca_res$score_2 - dmca_res$common_score_2
  ...
  structure(list(common_mat_1 = common_mat_1, common_mat_2 = common_mat_2, 
                 ...
                 common_score_1 = dmca_res$common_score_1, common_score_2 = dmca_res$common_score_2,
...
.mca_common_score <- function(svd_1, svd_2, mca_res,
  ...
  common_score_1 <- matrix(NA, nrow = n, ncol = K)
  common_score_2 <- matrix(NA, nrow = n, ncol = K)
  ...
  for(j in 1:K){
    ...
    common_score_1[,j] <- basis_res$basis_mat %*% tmp_decomp$common_vec1
    common_score_2[,j] <- basis_res$basis_mat %*% tmp_decomp$common_vec2
  }
  
  if(length(rownames(svd_1$u)) != 0) rownames(common_score_1) <- rownames(svd_1$u)
  if(length(colnames(svd_2$u)) != 0) rownames(common_score_2) <- colnames(svd_2$u)
  
  list(svd_1 = svd_1, svd_2 = svd_2, mca_res = mca_res,
       score_1 = score_1, score_2 = score_2,
       common_score_1 = common_score_1, common_score_2 = common_score_2)
}
